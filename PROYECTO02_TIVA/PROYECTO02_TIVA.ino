...
int push1State;
int push2State;
bool boton1Presionado = false; // Variable para indicar si el botón 1 fue presionado
int anim2 =0;
...
//***************************************************************************************************************************************
void loop() {
  int push1State = digitalRead(push1Pin);   //Lee el estado del boton 1 

  if (push1State == LOW) { // Si presiona el boton 1, verifica que hay datos disponibles en serial 2(UART ESP32)
    if (Serial2.available()) {
      ...
      if (newLatidosData > 0) {
        ...
        String textDato = "BPM: ";
        if (newLatidosData < 100) {  // Agrega un cero a la izquierda si el valor es menor que 100
          textDato += "0";
        }
        if (newLatidosData < 10) {  // Agrega un cero a la izquierda si el valor es menor que 10
          textDato += "0";
        }
        textDato += String(newLatidosData); // Agrega el valor, mostrara el mensaje en la TFT 
        LCD_Print(textDato, 155, 110, 2, 0x801F,  0x9CDF);
        lastLatidosData = newLatidosData;   //variable para guardar en el boton 2
        boton1Presionado = true;
        playpush2Melody(); //reproduce el sonidos
      }
  ...
  if (push2State == LOW) {
    if (boton1Presionado) {
      Mediciones = SD.open("hearo.txt", FILE_WRITE);  //lo pone en modo escritura
      if (Mediciones) {
        Mediciones.println(lastLatidosData); //guarda el dato en la micro SD 
        Mediciones.close(); //cierra el archivo
        Serial2.println("Toma de latidos guardada correctamente en hearo.txt");
        boton1Presionado = false;  //hace las tareas si esta presionado 
        playpush1Melody();
      } else {
  ...
  LCD_Sprite(15, 42, 128, 128, heart, 5, anim2, 0, 1);   //animacion de un corazon como un GIF
  anim2 = (anim2 + 1) % 5;
}
